...
public class SelectionPanelViewerTest {
    ...
    void selectionPanelTest() throws IOException, URISyntaxException, InterruptedException {
        ...
        Mockito.verify(guiMock).drawText(new Position(98, 101), 190, "Start", true);
        Mockito.verify(guiMock).drawText(new Position(98, 121), 190, "Exit", false);
        Mockito.verify(guiMock).drawText(new Position(140, 101), 190, "Continue", false);
        Mockito.verify(guiMock, Mockito.times(3)).drawText(
                Mockito.any(Position.class), Mockito.anyInt(), Mockito.anyString(), Mockito.anyBoolean());
    }
}
