...
public abstract class InteractableFactory {
    public static Interactable getInteractable(String type, int x, int y, int width, int height, String name) throws IOException {
        if ("Item".equalsIgnoreCase(type)) return new Item(x,y,width,height,name);
        else if("NPC".equalsIgnoreCase(type)) return new NPC(x,y,width,height,name);
